...
| **thread** | Keeper of a **multi-session work record** — one `THREAD_<topic>.md` carrying a long-running investigation or program across sessions. Auto-fires when you resume or update an existing record (never opens one on its own initiative) and enforces the record's contract: a resume-once, then work ⇄ integrate envelope with checkpoints, claims rewritten to current state (never old state plus what changed), state moving through a tracker, narrative history quarantined in a session log, and a fresh-eyes lint gating every checkpoint. **Use when** a thread of work outlives single sessions and its state must live in one structured document. |
...
- **thread** keeps the record of work that outlives single sessions: any session — hosted by attack-duck, data-investigation, or nothing — folds its yield back through thread's integrate step; it splits sub-threads via **handoff**, closes via **report**, and hands corpus promotion to **handoff-edit** → **apply-edit**.
