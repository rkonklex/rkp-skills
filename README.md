...
| **thread** | Keeper of a **multi-session work record** — one `THREAD_<topic>.md` carrying a long-running investigation or program across sessions. Auto-fires when you resume or update an existing record (never opens one on its own) and enforces its contract: revise-in-place, a work ⇄ integrate envelope with owner-armed checkpoints, owner-gated collapse of settled campaigns into `/report` micro-articles, a blast-radius sweep at substantive checkpoints, a fresh-eyes lint at every checkpoint. **Use when** a thread of work outlives single sessions and its state must live in one structured document. |
