...
| **thread** | Keeper of a **multi-session work record** — one `THREAD_<topic>.md` carrying a long-running investigation or program across sessions. Auto-fires when you resume or update an existing record (never opens one on its own initiative) and enforces the record's contract: a resume→integrate session envelope, corrections landing at the claim, state moving through a tracker, narrative history quarantined in a session log. **Use when** a thread of work outlives single sessions and its state must live in one structured document. |
