...
- **Label what you added.** In the NOTE, tag any claim that runs past the corpus or past solid knowledge by **origin**: `[inferred]` (you derived it), `[domain-knowledge]` (general expertise, uncited), `[speculation]` (a guess). Tag by how you came to hold the claim, not its inputs: a derived claim stays `[inferred]` even when its inputs were measured — never relabel it `[observed]`. The live chat needn't carry tags — except open mode, where the rail rides live (see Grounding) — but the record must.
