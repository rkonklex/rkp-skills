...
## Setup
1. **Mode + focus** from the argument. First token `continue`/`split` is the mode (infer and state it in one line if omitted). The rest is a short free-text description of what the handoff covers — use it as the focus and scope, and derive a kebab-case slug from it. If the argument is omitted, infer the focus from the conversation.
2. **Topic folder.** Save as `HANDOFF_<slug>.md` in the work's folder; the receiver's harness loads the corpus's rules — don't re-read it yourself. **Write the handoff in the language the host's documents use** (per those rules), not the chat's — English unless the corpus says otherwise. If a handoff for this topic already exists, **don't silently overwrite or spawn a near-duplicate** — update the existing file, or reconcile the slug with the owner. Duplicate handoffs fragment state.
...
- **Read first (in order).** Each source with why + section; code by path. Pointers only.
...
- **Cautions / out-of-scope.** Each with a pointer to where it belongs; carry forward any failure mode the host's rules declare.
